...
function safeTrackQuest(
    ...
    try {
        if (
            quest &&
            typeof quest.trackGambleResult ===
                "function"
        ) {
         ...
         */
        console.error(
            "[Limbo trackGambleResult]",
            error,
        );
    }
}

function buildRollingEmbed(
    interaction,
    bet,
    target,
) {
...
function buildResultEmbed({
    ...
    return new EmbedBuilder()
        ...
        .setFooter({
            text:
                "RTP 99% • Tiền nhận đã gồm vốn • " +
                "Hệ số tối đa 999,999x",
        })
        .setTimestamp();
}
...
class LimboManager {
    async play(interaction) {
        ...
        try {
            ...
            const rolledMultiplier =
                rollLimboMultiplier();

            const won =
                rolledMultiplier >= target;
